 ...
 * (MiMo-V2.6-Pro via the opencode-go provider, by default) to review a design,
 ...
 *
 * Cost: MiMo-V2.6-Pro on opencode-go (see opencode.ai/zen for current pricing).
 * A typical consult runs $0.002–0.01. The system
 ...
 * MiMo-V2.6-Pro), so repeated consults hit the endpoint's prefix cache at
 * the cacheRead rate. Default reasoning effort is
 * "max"; use "high" for a balanced, cheaper review and "low" for quick sanity
 * checks.
 *
...
/** Default consultant model (MiMo-V2.6-Pro). */
 ...
 *  prefix cache: cacheRead $0.0036/MTok vs $0.435/MTok input). */
...
export default function consultExtension(pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Consult",
		description:
			"Ask a frontier model (MiMo-V2.6-Pro) for expert feedback on a design, architecture, or plan. It stress-tests the proposal, surfaces risks, and suggests simpler alternatives. The consulted model sees only what you pass in — it has no access to the repo or conversation, so include the full proposal text and any relevant code/constraints.",
		promptSnippet:
			"Consult a frontier model (MiMo-V2.6-Pro) for expert feedback on a design, architecture, or plan",
		promptGuidelines: [
					...
					(["low", "high", "max"] as const).map((v) =>
						Type.Literal(v, {
							description: {
								...
								high: "Balanced depth and cost",
								max: "Deepest critique, slowest and most expensive (default)",
							}[v],
						}),
					),
				),
				{ description: "Thinking effort for the consulted model (default: max)" },
			),
		}),
